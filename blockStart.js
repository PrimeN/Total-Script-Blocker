...
//const NAME_TotalScriptBlocker_ALLOWED = "dQGIgCdSC1FWpDUMWx1z";
//const NAME_TotalScriptBlocker_WHITELIST = "WQQAW0JuSt0304877x4l";

const NAME_TotalScriptBlocker_ALLOWED = "cLjHCdSC1FWpDUMWfgo";
const NAME_TotalScriptBlocker_WHITELIST = "6ZtrW0JuSt0304877frg";
    ...
    get: function(key) {
        if ((PASSWORD_GOOD === PASSWORD_STATUS.okay) && this.has(key)) {
			...
			try {
				...
				if (OUT_PUT_LOG) console.error("Error in Total Script Blocker - blockStart.js (blockSettings.get): " + err);
    ...
    set: function(key, value) {
		if ((PASSWORD_GOOD === PASSWORD_STATUS.okay))
		{
			try {
				...
				fatalError = true;
				/*if (OUT_PUT_LOG) console.error("Error in Total Script Blocker - blockStart.js (blockSettings.set): " + err);
				if (err == QUOTA_EXCEEDED_ERR) {
					alert('localStorage quota exceeded for Total Script Blocker on ' + window.location.href);
				}*/
			}
		}
		else
...
// Note: We converted to a JSON representation in Total Script Blocker V0.9.6.
// Before that it was an array so we have to detect and upgrade older data without deleting it.
if (!blockSettings.has(NAME_TotalScriptBlocker_WHITELIST)) {
	blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, genEmptyWhitelist());
}
var TotalScriptBlocker_Whitelist = blockSettings.get(NAME_TotalScriptBlocker_WHITELIST);
if (isArray(TotalScriptBlocker_Whitelist))	// Need to convert to new JSON format and sort
{
	if (OUT_PUT_LOG) 
		console.log("site whitelist isArray");
	if (!sortUrlList(TotalScriptBlocker_Whitelist))	// in place sort
	{
		// There was an error sorting that requires us to reset the whitelist
		TotalScriptBlocker_Whitelist = genEmptyWhitelist();
		blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, TotalScriptBlocker_Whitelist);
	}
	else
	{
		// The whitelistHash will be updated when we get the settings from the background page
		var emptyList = genEmptyWhitelist();
		emptyList.whitelist = TotalScriptBlocker_Whitelist;
		emptyList.whitelistHash = "";
		TotalScriptBlocker_Whitelist = emptyList;
		blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, TotalScriptBlocker_Whitelist);
	}
}
else if (typeof TotalScriptBlocker_Whitelist === 'undefined' || TotalScriptBlocker_Whitelist === null
	|| !isArray(TotalScriptBlocker_Whitelist.whitelist) || !isArray(TotalScriptBlocker_Whitelist.tempAllowList))	// data is corrupt
{
	if (OUT_PUT_LOG) 
...
if (TotalScriptBlocker_Whitelist.tempExpiry > 0 && TotalScriptBlocker_Whitelist.tempExpiry < CURR_EPOCH)
{
	if (OUT_PUT_LOG) 
		console.log("temp list expired");
	TotalScriptBlocker_Whitelist.tempAllowList = [];
	TotalScriptBlocker_Whitelist.tempAllowListHash = EMPTY_MD5;
	TotalScriptBlocker_Whitelist.globalAllowAll = false;
	TotalScriptBlocker_Whitelist.tempExpiry = 0;
	blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, TotalScriptBlocker_Whitelist);
}
...
function resetAllStorage(settings)
{
	...
	{
		fatalError = false;
		blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, settings.whitelist);
		TotalScriptBlocker_Whitelist = blockSettings.get(NAME_TotalScriptBlocker_WHITELIST);
	}
...
{
	return (blocking_mode === BMODE_TYPES.WHITELIST_ALLOW_TOP_LEVEL ? islisted(TotalScriptBlocker_Whitelist.blacklist, url) : false);
}
...
function isWhitelisted(url) {
	switch(blocking_mode)
	{
		...
		{
			return !islisted(TotalScriptBlocker_Whitelist.blacklist, url);
		}
		case BMODE_TYPES.WHITELIST_ALLOW_TOP_LEVEL:
		{
			if (!islisted(TotalScriptBlocker_Whitelist.blacklist, url))
			{
				return (patternMatches(url, topDomain)) || islisted(TotalScriptBlocker_Whitelist.whitelist, url);
			}
			else
				return false;
		}
		default:	// BMODE_TYPES.WHITELIST
			return islisted(TotalScriptBlocker_Whitelist.whitelist, url);
	}	
}

function isTempAllowListed(url) {	
	return islisted(TotalScriptBlocker_Whitelist.tempAllowList, url);
}

function isGloballyAllowed()
{
	return TotalScriptBlocker_Whitelist.globalAllowAll;
}
...
function blockScripts(event)
{
	...
	*/
		...
		(!do_not_block_scripts && (elType === EL_TYPE.SCRIPT)))
	{
		...
		if (OUT_PUT_LOG) 
			console.log("After " + "   " + mainURL + "   " + TotalScriptBlocker_Allowed.globalAllowAll + "    " + isWhitelisted(mainURL) + "   " + isTempAllowListed(mainURL));
...
function updateSettings(settings)
{
	...
	if (TotalScriptBlocker_Whitelist.whitelistHash !== settings.whitelist.whitelistHash
		|| TotalScriptBlocker_Whitelist.tempAllowListHash !== settings.whitelist.tempAllowListHash
		|| TotalScriptBlocker_Whitelist.blacklistHash !== settings.whitelist.blacklistHash
		|| TotalScriptBlocker_Whitelist.globalAllowAll !== settings.whitelist.globalAllowAll
		|| TotalScriptBlocker_Whitelist.tempExpiry !== settings.whitelist.tempExpiry)
	{
		blockSettings.set(NAME_TotalScriptBlocker_WHITELIST, settings.whitelist);
		TotalScriptBlocker_Whitelist = settings.whitelist;
		
		if (TotalScriptBlocker_Whitelist.globalAllowAll && pageSourcesForbidden.length > 0)
		{
			needToReload = true;
		}		
	}
	...
	if (OUT_PUT_LOG) 
		console.log("after TotalScriptBlocker_Whitelist.whitelist: " + TotalScriptBlocker_Whitelist.whitelist);
	...
	if (msg.type === "get sources")
	{
		if (window.top === window)
		{
			send({"globalAllowAll": isGloballyAllowed(), 
				...
				"topDomainIsWhitelisted": (blocking_mode === BMODE_TYPES.WHITELIST_ALLOW_TOP_LEVEL ? islisted(TotalScriptBlocker_Whitelist.whitelist, topDomain) : false) });
		}
		else
		{
